...
public class MedicalRecordRepository {
    ...
    public List<MedicalRecordEntry> findByPatientId(Long patientId) throws SQLException {
        String sql = """
                SELECT e.entry_id,
                       e.file_id,
                       f.patient_id,
        ...
        List<MedicalRecordEntry> result = new ArrayList<>();
        try (Connection conn = Repository.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    result.add(new MedicalRecordEntry(
                            rs.getLong("entry_id"),
                            rs.getLong("file_id"),
                            rs.getLong("patient_id"),
                            rs.getObject("appointment_id", Long.class),
