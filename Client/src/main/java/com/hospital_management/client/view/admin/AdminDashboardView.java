...
import com.hospital_management.client.presenter.admin.AdminUsersPresenter;
...
public class AdminDashboardView {
    ...
    @FXML private Label totalDoctorsValueLabel;
    @FXML private Label totalPatientsValueLabel;
    @FXML private Label activeAccountsValueLabel;
    @FXML private Label newAccountsValueLabel;
    @FXML private Label totalDoctorsTrendLabel;
    @FXML private Label totalPatientsTrendLabel;
    @FXML private Label activeAccountsTrendLabel;
    @FXML private Label newAccountsTrendLabel;

    @FXML private Button staffTabButton;
    @FXML private Button patientsTabButton;
    ...
    @FXML private javafx.scene.control.DatePicker appointmentsFilterDatePicker;
    ...
    public void onNavDashboardClick() {
        presenter.loadStats();
        presenter.loadUsers();
        setInfo("Datele au fost actualizate.");
    }

    @FXML
    public void onNavUsersClick() {
        setInfo("Sunteti in sectiunea de administrare utilizatori.");
    }
    ...
    @FXML
    public void onNavDepartmentsClick() {
        setInfo("Sectiunea Departamente este in lucru.");
    }
    ...
    @FXML
    public void onStaffTabClick() {
        if (currentTab != TabSelection.STAFF) {
            currentTab = TabSelection.STAFF;
            updateTabStyles();
            applyFilters();
        }
    }
    ...
    @FXML
    public void onPatientsTabClick() {
        if (currentTab != TabSelection.PATIENTS) {
            currentTab = TabSelection.PATIENTS;
            updateTabStyles();
            applyFilters();
        }
    }
    ...
    public void setSpecializations(List<SpecializationDTO> items) {
        ...
        departmentFilter.getItems().add(ALL_DEPARTMENTS);
        for (SpecializationDTO dto : specializations) {
            departmentFilter.getItems().add(dto.getName());
        }
    ...
    private HBox buildAppointmentActions(AppointmentDTO appt) {
        ...
        String status = normalizeAppointmentStatus(appt.getStatus());
        if ("DONE".equals(status) || "CANCELED".equals(status)) {
            return actions;
        }

        actions.getChildren().add(createActionButton("Modifica", "table-action", () -> openEditAppointment(appt)));
        actions.getChildren().add(createActionButton("Anuleaza", "table-action-danger", () -> presenter.cancelAppointment(appt.getAppointmentId())));
        return actions;
    }
    ...
    private void renderUserHeader(UserDTO user) {
        if (user == null) {
            ...
            adminInitialsLabel.setText("AD");
            return;
        }
        String fullName = user.getFullName();
        adminNameLabel.setText(fullName);
        adminRoleLabel.setText(roleLabel(user.getRole()));
        adminInitialsLabel.setText(initials(fullName));
    }
    ...
    private void applyFilters() {
        ...
        for (AdminUserDTO user : allUsers) {
            ...
            if (!matchesDepartment(user, department)) {
                continue;
            }
            if (!matchesStatus(user, status)) {
                continue;
            }
    ...
    private void refreshTable() {
        ...
        for (int i = start; i < end; i++) {
            usersContainer.getChildren().add(buildUserRow(filteredUsers.get(i)));
        }
    ...
    private int totalPages() {
        return Math.max(1, (int) Math.ceil(filteredUsers.size() / (double) PAGE_SIZE));
    }
    ...
    private GridPane buildUserRow(AdminUserDTO user) {
        ...
        HBox nameCell = new HBox(8.0);
        nameCell.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        StackPane avatar = new StackPane();
        avatar.getStyleClass().add("table-avatar");
        Label initials = new Label(initials(user.getFullName()));
        initials.getStyleClass().add("table-avatar-text");
        avatar.getChildren().add(initials);
        VBox nameBox = new VBox(2.0);
        Label nameLabel = new Label(valueOrDefault(user.getFullName(), "Utilizator"));
        nameLabel.getStyleClass().add("table-text");
        Label emailLabel = new Label(valueOrDefault(user.getEmail(), "-"));
        emailLabel.getStyleClass().add("table-text-muted");
        nameBox.getChildren().addAll(nameLabel, emailLabel);
        nameCell.getChildren().addAll(avatar, nameBox);

        VBox roleCell = new VBox(2.0);
        Label roleLabel = new Label(roleLabel(user.getRole()));
        roleLabel.getStyleClass().add("table-text");
        Label departmentLabel = new Label(valueOrDefault(user.getDepartment(), "-"));
        departmentLabel.getStyleClass().add("table-text-muted");
        roleCell.getChildren().addAll(roleLabel, departmentLabel);

        Label activityLabel = new Label(formatDateTime(user.getLastActivityDate(), user.getLastActivityTime()));
        activityLabel.getStyleClass().add("table-text-muted");

        Label status = new Label(statusLabel(user.getStatus()));
        status.getStyleClass().add("status-pill");
        status.getStyleClass().add(statusStyle(user.getStatus()));

        HBox actions = new HBox(6.0);
        Button editButton = new Button("Editeaza");
        editButton.getStyleClass().add("table-action");
        editButton.setOnAction(event -> showUserDialog(user));
        Button deleteButton = new Button("Sterge");
        deleteButton.getStyleClass().addAll("table-action", "table-action-danger");
        deleteButton.setOnAction(event -> confirmDelete(user));
        actions.getChildren().addAll(editButton, deleteButton);

        row.add(nameCell, 0, 0);
        row.add(roleCell, 1, 0);
        row.add(activityLabel, 2, 0);
        row.add(status, 3, 0);
        row.add(actions, 4, 0);

        GridPane.setHgrow(nameCell, Priority.ALWAYS);
        GridPane.setHgrow(actions, Priority.ALWAYS);
    ...
    private void confirmDelete(AdminUserDTO user) {
        ...
        alert.setHeaderText("Stergi utilizatorul " + user.getFullName() + "?");
        alert.setContentText("Actiunea nu poate fi anulata.");
    ...
    private void showUserDialog(AdminUserDTO user) {
        ...
        roleBox.getItems().addAll(roleOptions);
        roleBox.setConverter(new StringConverter<>() {
            @Override
            public String toString(RoleOption object) {
                return object == null ? "" : object.label;
            }

            @Override
            public RoleOption fromString(String string) {
                return roleOptions.stream()
                        .filter(option -> Objects.equals(option.label, string))
                        .findFirst()
                        .orElse(null);
            }
        });

        ComboBox<SpecializationDTO> specializationBox = new ComboBox<>();
        specializationBox.getItems().addAll(specializations);
        specializationBox.setConverter(new StringConverter<>() {
            @Override
            public String toString(SpecializationDTO object) {
                return object == null ? "" : object.getName();
            }

            @Override
            public SpecializationDTO fromString(String string) {
                return specializations.stream()
                        .filter(spec -> Objects.equals(spec.getName(), string))
                        .findFirst()
                        .orElse(null);
            }
        });
        ...
        int row = 0;
        grid.add(new Label("Prenume"), 0, row);
        grid.add(firstNameField, 1, row++);
        grid.add(new Label("Nume"), 0, row);
        ...
        Label specializationLabel = new Label("Specializare");
        grid.add(specializationLabel, 0, row);
        grid.add(specializationBox, 1, row++);

        Label nationalIdLabel = new Label("CNP");
        grid.add(nationalIdLabel, 0, row);
        ...
        if (isEdit) {
            ...
            if (user.getSpecializationId() != null) {
                specializationBox.getSelectionModel().select(findSpecialization(user.getSpecializationId()));
            }
        ...
        roleBox.valueProperty().addListener((obs, oldVal, newVal) ->
                applyRoleVisibility(newVal, specializationLabel, specializationBox, nationalIdLabel, nationalIdField));

        Runnable validate = () -> {
            boolean basic = !firstNameField.getText().isBlank()
                    && !lastNameField.getText().isBlank()
                    && !emailField.getText().isBlank()
                    && roleBox.getValue() != null;
            boolean needsPassword = !isEdit && passwordField.getText().isBlank();
            boolean doctorNeedsSpec = "DOCTOR".equalsIgnoreCase(roleValue(roleBox.getValue()))
                    && specializationBox.getValue() == null;
            okButton.setDisable(!basic || needsPassword || doctorNeedsSpec);
        };

        firstNameField.textProperty().addListener((obs, oldVal, newVal) -> validate.run());
        lastNameField.textProperty().addListener((obs, oldVal, newVal) -> validate.run());
        emailField.textProperty().addListener((obs, oldVal, newVal) -> validate.run());
        passwordField.textProperty().addListener((obs, oldVal, newVal) -> validate.run());
        roleBox.valueProperty().addListener((obs, oldVal, newVal) -> validate.run());
        specializationBox.valueProperty().addListener((obs, oldVal, newVal) -> validate.run());
        validate.run();
        ...
        dialog.setResultConverter(button -> {
            ...
            RoleOption role = roleBox.getValue();
            SpecializationDTO specialization = specializationBox.getValue();
            UserFormData data = new UserFormData();
            data.userId = isEdit ? user.getUserId() : null;
            data.firstName = firstNameField.getText().trim();
            data.lastName = lastNameField.getText().trim();
    ...
    private void applyRoleVisibility(RoleOption role,
                                     Label specializationLabel,
                                     ComboBox<SpecializationDTO> specializationBox,
                                     Label nationalIdLabel,
                                     TextField nationalIdField) {
        String value = roleValue(role);
        boolean isDoctor = "DOCTOR".equalsIgnoreCase(value);
        boolean isPatient = "PATIENT".equalsIgnoreCase(value);

        specializationLabel.setManaged(isDoctor);
        specializationLabel.setVisible(isDoctor);
        specializationBox.setManaged(isDoctor);
        specializationBox.setVisible(isDoctor);

        nationalIdLabel.setManaged(isPatient);
        nationalIdLabel.setVisible(isPatient);
        nationalIdField.setManaged(isPatient);
        nationalIdField.setVisible(isPatient);
    }

    private RoleOption roleOptionFor(String role) {
        return roleOptions.stream()
                .filter(option -> option.value.equalsIgnoreCase(role))
                .findFirst()
                .orElse(null);
    }

    private String roleValue(RoleOption option) {
        return option == null ? "" : option.value;
    }

    private SpecializationDTO findSpecialization(Long id) {
        if (id == null) {
            return null;
        }
        for (SpecializationDTO dto : specializations) {
    ...
    private boolean matchesTab(AdminUserDTO user) {
        if (user == null) {
            return false;
        }
        if (currentTab == TabSelection.PATIENTS) {
    ...
    private boolean matchesSearch(AdminUserDTO user, String search) {
        if (search.isBlank()) {
            return true;
        }
        String fullName = normalize(user.getFullName());
        String email = normalize(user.getEmail());
        String nationalId = normalize(user.getNationalId());
    ...
    private boolean matchesDepartment(AdminUserDTO user, String department) {
        if (department == null || department.isBlank() || ALL_DEPARTMENTS.equals(department)) {
            return true;
        }
        return Objects.equals(user.getDepartment(), department);
    }

    private boolean matchesStatus(AdminUserDTO user, String status) {
        if (status == null || status.isBlank() || ALL_STATUS.equals(status)) {
            return true;
        }
        return Objects.equals(statusLabel(user.getStatus()), status);
    }
    ...
    private String formatDateTime(LocalDate date, LocalTime time) {
        ...
        DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("dd MMM yyyy", Locale.forLanguageTag("ro"));
    ...
    private void setupAppointmentsStatusFilter() {
        ...
        appointmentsStatusFilter.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) ->
                presenter.onAppointmentsTabSelected(statusCodeForAppointmentLabel(newVal)));
    }
    ...
    private String labelForAppointmentStatus(String statusCode) {
        String normalized = normalizeAppointmentStatus(statusCode);
        return switch (normalized) {
            case "PENDING" -> "In asteptare";
    ...
    private void updateTabStyles() {
        if (staffTabButton == null || patientsTabButton == null) {
            return;
        }
        staffTabButton.getStyleClass().remove("tab-active");
        patientsTabButton.getStyleClass().remove("tab-active");
        if (currentTab == TabSelection.STAFF) {
            staffTabButton.getStyleClass().add("tab-active");
        } else {
            patientsTabButton.getStyleClass().add("tab-active");
        }
    }
    ...
    private static class RoleOption {
        ...
        private RoleOption(String label, String value) {
            this.label = label;
            this.value = value;
        }
    }
