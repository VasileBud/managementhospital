...
import view.patient.PatientDashboardView;
...
public class PatientDashboardPresenter {
    ...
    public void onCancelAppointment(long appointmentId) {
        ...
        if(appointmentId <= 0) {
            view.setError("Eroare, date invalide!");
        }
    ...
    private void loadPatientDetails(long patientId) {
        ...
        ClientSession.getInstance().getClient().sendRequest(req, response -> {
            Platform.runLater(() -> {
                ...
                PatientProfileDTO details = response.getData() instanceof PatientProfileDTO
                        ? (PatientProfileDTO) response.getData()
                        : null;
