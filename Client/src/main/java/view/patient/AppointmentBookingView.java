...
import presenter.patient.AppointmentBookingPresenter;
...
public class AppointmentBookingView {
    ...
    private boolean updatingBookingDetails = false;
    private String currentRole = "PATIENT";
    private List<LocalTime> allAvailableTimes = List.of();
    ...
    public void setPatients(List<AdminUserDTO> patients) {
        ...
        clearSelectionLater(patientCombo);
    }

    public void setDoctors(List<DoctorDTO> doctors) {
        doctorCombo.getItems().setAll(doctors);
        clearSelectionLater(doctorCombo);
    }
    ...
    public void setServices(List<MedicalServiceDTO> services) {
        ...
        clearSelectionLater(serviceCombo);
    }
    ...
    public void setAvailableTimes(List<LocalTime> times) {
        allAvailableTimes = times == null ? List.of() : List.copyOf(times);
        renderTimeButtons(allAvailableTimes, false);
    }
    ...
    private void updateBookingDetails() {
        if (bookingDetailsLabel == null || updatingBookingDetails) {
            return;
        }
        updatingBookingDetails = true;
        try {
    ...
    private void clearSelectionLater(ComboBox<?> combo) {
        if (combo == null) {
            return;
        }
        Platform.runLater(() -> combo.getSelectionModel().clearSelection());
    }

    private void renderTimeButtons(List<LocalTime> times, boolean filtered) {
        timeSlotsPane.getChildren().clear();
        if (times == null || times.isEmpty()) {
            Label empty = new Label(filtered ? "Nu exista ore pentru filtrul introdus." : "Nu exista ore disponibile.");
